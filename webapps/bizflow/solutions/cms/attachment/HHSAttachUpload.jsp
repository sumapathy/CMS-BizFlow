...
<bf:parameter id="processId" name="processId" value="0" valuePattern="Numeric"/>
<bf:parameter id="workId" name="workId" value="0" valuePattern="Numeric"/>													
	...
	public String saveEdmsMetadata(String jsonStr) throws Exception
	{
		...
		try {
            ...
            for(int i=0; i < ja.length(); i++) {
                ...
                {
                    xrs.setFieldValueAt(index, "DISPLAYNAME", jo.optString("name"));
                } 
...
try
{
	...
	BasicHandler basicWih = null;
    int procId = 0;
    int actseq = 0;
    int workseq = 0;
    boolean foundIds = false;
    if("0".equals(processId) && "0".equals(workId)) {
        if(!hwiniSystem.getBoolValue("WORK_AREA_DEFAULT", "ENABLE_OPEN_MULTIPLE_WIH", false)) {
            String latestOpenedWorkitem = (String) session.getAttribute("latestOpenedWorkitem");
            if (null != latestOpenedWorkitem && latestOpenedWorkitem.length() > 0) {
                String[] latestOpenedWorkitemIds = com.hs.frmwk.web.util.StringUtility.split(latestOpenedWorkitem, "_");
                if (latestOpenedWorkitemIds.length == 3) {
                    procId = Integer.parseInt(latestOpenedWorkitemIds[0]);
                    actseq = Integer.parseInt(latestOpenedWorkitemIds[1]);
                    workseq = Integer.parseInt(latestOpenedWorkitemIds[2]);
                    foundIds = true;
                }
            }
        }
    ...
    if(!foundIds) {
        basicWih = getBasicHandler(request, procId, workseq, "y".equals(basicWihReadOnly)); //BF-15339
        procId = basicWih.getProcessId();
        actseq = basicWih.getActivityId();
        workseq = basicWih.getWorkId();
    }																							
